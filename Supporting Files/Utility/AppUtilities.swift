...
class AppUtilities: NSObject {
    ...
    func checkUpdate(vc: UIViewController) {
        if !userDefault.bool(forKey: UserDefaults.Keys.remindLater.rawValue) {
    ...
    func nextReminderForUpdate() {
        let nextDate = userDefault.value(forKey: UserDefaults.Keys.nextDate.rawValue) as? Date ?? Date()
        if Date() >= nextDate {
            userDefault.setValue(false, forKey: UserDefaults.Keys.remindLater.rawValue)
        }
    }
    func showAlertUpdate(title: String, msg: String, view: UIViewController) {
        ...
        let later = UIAlertAction(title: remindMeLater, style: .default) { UIAlertAction in
            ...
            debugPrint(nextDate)
            userDefault.setValue(nextDate, forKey: UserDefaults.Keys.nextDate.rawValue)
            userDefault.setValue(true, forKey: UserDefaults.Keys.remindLater.rawValue)
        }
